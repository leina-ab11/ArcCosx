...
def read_input(user_input):
    ...
    input when the user is out of the domain [-1,1]
    ...
    try:
        ...
        raise ValueError(
            f"Syntax error: '{user_input}' is not a number."
            f" Enter a real number, e.g. 0.5."
        ) from exc
    if x < -1 or x > 1 or is_nan(x):
        raise ValueError(
           f"Domain error: arccos is undefined for x = {x}. "
           f"Valid domain: [-1, 1]."
        )
    return x
